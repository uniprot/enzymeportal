...
public class EnzymePortalService {
    ...
    @Transactional(readOnly = true)
    public Page<UniprotEntry> filterBySpecieAndAnyOf(Long taxId, String compoudId, String meshId, Pageable pageable) {

        return uniprotEntryRepository.findAll(filterBySpecieAndAnyOf(taxId, compoudId, meshId), pageable);
    }

    @Transactional(readOnly = true)
    public Page<UniprotEntry> filterBySpecieAndCompoundAndDisease(Long taxId, String compoudId, String meshId, Pageable pageable) {

        return uniprotEntryRepository.findAll(filterBySpecieAndCompoundAndDisease(taxId, compoudId, meshId), pageable);
    }

    private static Predicate filterBySpecieAndAnyOf(Long taxId, String compoudId, String meshId) {

        QUniprotEntry enzyme = QUniprotEntry.uniprotEntry;

        Predicate compound = enzyme.enzymePortalCompoundSet.any().compoundId.equalsIgnoreCase(compoudId);
        Predicate disease = enzyme.enzymePortalDiseaseSet.any().meshId.equalsIgnoreCase(meshId);

        Predicate predicate = enzyme.taxId.eq(taxId).andAnyOf(compound, disease);

        return predicate;
    }

    private static Predicate filterBySpecieAndCompoundAndDisease(Long taxId, String compoudId, String meshId) {

        QUniprotEntry enzyme = QUniprotEntry.uniprotEntry;

        Predicate predicate = enzyme.taxId.eq(taxId).
                and(enzyme.enzymePortalCompoundSet.any().compoundId.equalsIgnoreCase(compoudId))
                .and(enzyme.enzymePortalDiseaseSet.any().meshId.equalsIgnoreCase(meshId));
