...
public class EnzymePortalEcNumbersRepositoryImpl implements EnzymePortalEcNumbersRepositoryCustom{
    ...
    public List<String> findAccessionsByEc(String ecNumber) {
      ...
      // List<String> enzymes = query.from($).where($.ecNumber.equalsIgnoreCase(ecNumber)).list($.uniprotAccession.accession);
          
       
              
              
              
              List<EnzymePortalEcNumbers> ecs = query.from($).where($.ecNumber.equalsIgnoreCase(ecNumber)).distinct().list($).stream().distinct().collect(Collectors.toList());
