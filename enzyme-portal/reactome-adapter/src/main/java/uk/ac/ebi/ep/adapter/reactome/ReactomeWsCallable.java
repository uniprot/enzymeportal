...
public class ReactomeWsCallable implements Callable<ReactionPathway> {
    ...
    private static final String REPLACEMENT = "<a href='http://www.reactome.org/";
...
//		return pathway;
    ...
    public String getDescription(ReactomeClass reactomeClass, String reactomeId)
        ...
        String desc = null;
        StringBuilder sb = new StringBuilder(config.getWsBaseUrl()).append(reactomeClass.name()).append('/').append(reactomeId);
        InputStream is = null;
        try {
            ...
            sb = new StringBuilder(config.getWsBaseUrl()).append(ReactomeClass.Summation.name()).append('/').append(handler.getSummationDbid());
