 ...
 */
public class EbeyeProteinService {
    ...
    public List<Protein> queryForUniqueProteins(String query, int limit) {
        ...
        return queryForUniqueProteins(query)
                .limit(limit)
                .toList()
                .toBlocking()
                .single();
    ...
    public Observable<Protein> queryForUniqueProteins(String query) {
        ...
        try {
            ...
            if (hitCount <= ebeyeIndexUrl.getMaxEbiSearchLimit()) {
                uniqueProteins = Observable.from(firstSearchResult.getEntries())
    ...
    private Observable<URI> generateUrlRequests(int start, int end, String query) {
        final String ebeyeProteinQuery = "%s?query=INTENZ:%s&size=%d&start=%d&fields=id,name,scientific_name&format=json";
        ...
        return Observable.range(start, end)
                .map(index
                        -> URI.create(String.format(ebeyeProteinQuery, endpoint, query, resultSize,
    ...
    private Observable<Protein> getDistinctProteinsFromEntries(Observable<Entry> accessionObservable) {
