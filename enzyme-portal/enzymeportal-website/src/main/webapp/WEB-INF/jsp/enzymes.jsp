            ...
            <div id="content" role="main" class="clearfix">
                ...
                <div class="row" id="title-row">
                    ...
                    <section class="large-10 columns">
                        ...
                        --%>
                        <c:choose>
                            <c:when test="${ebiResult.hitCount eq 0}">
                                <h4>No result found for "${searchKey}"</h4>   
                            </c:when>
                            <c:when test="${ebiResult.hitCount gt 1}">
                                <h4>${ebiResult.hitCount} results found for "${searchKey}"</h4>   
                            </c:when>
                            <c:otherwise>
                                <h4>${ebiResult.hitCount} result found for "${searchKey}"</h4>   
                            </c:otherwise>
                        </c:choose>
                                ...
                                <c:forEach var="facet" items="${enzymeFacet}">
                                    <div>
                                        ...
                                        <c:if test="${facet.id eq 'enzyme_family'}">
                                            <div class="sublevel1">
                                                <div class="subTitle">Enzyme Class</div>
                                                <ul>
                                    ...
                                    <c:forEach var="enzyme" items="${enzymeView}" varStatus="theIndex">
                                        ...
                                        <input type="checkbox" class="forBasket"
                                            ...
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <td> <a href="${pageContext.request.contextPath}/ec/${enzyme.ec}">${enzyme.enzymeName}</a></td>
                                                    </c:otherwise>
                                  ...
                                  <td>
                                      <div class="alternative-names">
                                          <ul>
                                            ...
                                            <td>
                                                <div class="catalytic_Activities">
                                                    <ul>
                                                        <c:forEach var="cat" items="${enzyme.catalyticActivities}">
                                                            <li>${cat}</li>
                                                            </c:forEach>
                                                    </ul>
                                                </div>
                                        ...
                                        <c:if test='${enzyme.numProteins > 0}'>
                                            <tr id="proteinList${theIndex.index}" data-toggler data-animate="hinge-in-from-top hinge-out-from-top" style="display: none">
                                                <td colspan="7">
                                                    <form id="proteinViewForm-${enzyme.ec}" action="${pageContext.request.contextPath}/search" modelAttribute="searchModel" method="POST">
                                                        ...
                                                        <ul id="enzymeResultsProteins">
                                                           ...
                                                           <c:choose>
                                                                    ...
                                                                    <c:when test="${keywordType eq 'COFACTORS'}">
                                                                        ...
                                                                        <c:set var="primaryProtein" value="${epfn:withCofactor(p.withCofactor, searchId, p.primaryAccession, p.primaryOrganism,p.entryType)}"/>
                                                                        ...
                                                                        <c:when test="${keywordType eq 'DISEASE'}">
                                                                            <c:set var="primaryProtein" value="${p.withDisease}"/>
                                                                        <li><a href="${pageContext.request.contextPath}/search/${primaryProtein.accession}/diseaseDrugs">${p.proteinName} </a> - (${primaryProtein.commonName})</li>
                                                                        </c:when>
                                                                        <c:when test="${keywordType eq 'TAXONOMY'}">
                                                                            <c:set var="primaryProtein" value="${epfn:withTaxonomy(p.withTaxonomy, searchId, p.primaryAccession, p.primaryOrganism,p.entryType)}"/>
                                                                        <li><a href="${pageContext.request.contextPath}/search/${primaryProtein.accession}/enzyme">${p.proteinName} </a> - (${primaryProtein.commonName})</li>
                                                                        </c:when>
                                                                        <c:when test="${keywordType eq 'PATHWAYS'}">
                                                                            <c:set var="primaryProtein" value="${epfn:withPathway(p.withPathway, searchId, p.primaryAccession, p.primaryOrganism,p.entryType)}"/>
