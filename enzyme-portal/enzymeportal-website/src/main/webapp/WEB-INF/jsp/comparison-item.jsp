...
<c:choose>
    ...
    <c:when test="${fn:contains(comparePath, 'EC classification')
                    and not empty item}">
            <a href="${pageContext.request.contextPath}/ec/${item}" target="_blank">EC ${item}</a>
            ...
            <c:if test="${not empty item.reaction.id}">
                ...
                <div>
                    ...
                    <a href="${rheaEntryBaseUrl}${item.reaction.id}"
                       target="_blank">${item.reaction.id}</a>
                </div>
            </c:if>
            <c:if test="${not empty item.pathways}">
                <div>
                    ...
                    <c:forEach var="pathway" items="${item.pathways}" varStatus="vs">
                        <a href="https://www.reactome.org/content/detail/${pathway}"
                           target="_blank">${pathway}</a>${vs.last? '' : ','}
                    </c:forEach>
                </div>
            </c:if>
        </div>
